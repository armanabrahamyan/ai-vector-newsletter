...
- **One defect counts once.** The reviewer files a wrong figure at every place it appears (headline, digest bullet, summary, take) and sometimes files one sentence under two criteria. Code folds those repeats: the extra findings stay in the report marked `echo of f00X, not counted`, and only the first-counted one reaches the threshold table. The verdict line shows how many were folded (`2 echo(es) not counted`). You still fix every location; you are just not held for one mistake three times.
