...
- **Schema:** a single `ReviewReport` object (not JSONL). The keys the rest
  ...
  - `echo_of` on a finding (since 2026-08-29) — set by code when the
    finding is the same defect as another one: the same quoted span under
    a second criterion, or a factual / reputational finding on another
    field of the same story (a digest bullet citing exactly one story
    counts as that story's field). An echo stays in `findings` — the
    reviser still needs every location — but **only findings without
    `echo_of` feed the severity tally the threshold table reads.** One
    wrong number copied into headline, digest and take is one defect.
...
| 2026-08-29 | `ReviewFinding`, `ReviewReport` | v1 / v3 | v2 / v4 | `ReviewFinding` gains optional `echo_of: str \| None = None` (a finding id). Set by code (`review.group_echoes`) when the finding is the same defect as another: same target field + same quoted span under any criterion, or a `factual_grounding` / `reputational_liability` finding on any field of the same story (digest bullets citing exactly one story join that story). `ReviewReport.severity_counts()` now tallies only findings with `echo_of` unset; new `echo_count()`. `ReviewReport` v4 is the envelope bump. Motivation: over ten reviews one mistake was routinely filed three or four times (once per field it appeared in) and tripped the "three majors → red" rule on its own. | No on-disk migration. Archived `review.json` files carry no `echo_of`, so every finding in them counts exactly as when written. `review.md` frontmatter gains `findings_echoes`; `findings_by_severity` now reports counted findings only. `src/revise.py` reads `findings` unchanged and still sees every location. |
